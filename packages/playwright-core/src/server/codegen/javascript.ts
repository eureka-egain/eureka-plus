...
${useText ? '\ntest.use(' + useText + ');\n' : ''}
    ...
    if (options.contextOptions.recordHar) {
      ...
      /** MOD:
       * We get absolute path from the cli script
       * which we need to convert to relative path
       * to be able to run the test from any directory.
       * The har file should be saved in the same directory as the test
       */
      const absolutePath = options.contextOptions.recordHar.path;
      const harFileName = path.basename(absolutePath);
      formatter.add(
          `  await page.clock.setFixedTime(new Date('2022-02-02T10:00:00'));`
      );
      formatter.add(
          `  await page.routeFromHAR(${`path.resolve(__dirname, '${harFileName}')`}${
            url ? `, ${formatOptions({ url }, false)}` : ''
          });`
      );
    }
    return formatter.format();
  ...
  options.browserName
    ...
    if (options.contextOptions.recordHar) {
      formatter.add(
          `        await context.routeFromHAR(${quote(
              options.contextOptions.recordHar.path
          )});`
      );
    }
    return formatter.format();
